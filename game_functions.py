...
def update_screen(ai_settings, screen, stats, sb, ship, aliens,
    ...
    [bullet.blitme() for bullet in bullets.sprites()]
    ship.blitme()
    aliens.draw(screen)
    sb.show_score()
...
def remember_high_score(stats):
    hs = take_high_score(stats)
    if hs == '' or stats.high_score > int(hs):
        with open('text_files/high_score.txt', 'w') as ouf:
            ouf.write(str(stats.high_score))


def take_high_score(stats):
    with open('text_files/high_score.txt') as inf:
        return inf.read()


def play_music(stats):
    pygame.mixer.music.load('sound/Hug-You-Hardcore.ogg')
